...
on run
	...
	set choosen_items to choose file with prompt "Dateien zur Umbennenung auswählen:" with multiple selections allowed
...
on processFiles(choosen_files)
	...
	-- Ask for Prefix
	set confirm_dialog to display dialog ¬
		"Vor den Dateinamen der ausgewählten" & space & choosen_amount & space & "Dateien anfügen:" default answer ¬
		CurrentTimeString() & "_" buttons {"Abbrechen", "Anfügen"} default button "Anfügen" with title "DropAddDate" giving up after 20
	-- Rename Files if not canceled
	if button returned of confirm_dialog is "Anfügen" then
